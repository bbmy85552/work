...
export default function CustomerManagementClient({ initialSchools = [], initialStatistics, regions: regionOptions = [] }) {
  ...
  const isCooperationChecked = Form.useWatch('isCooperation', form)
  const policyCancelRef = useRef(null)
  const policyChunksRef = useRef([])

  const allSchools = useMemo(() => {
    if (!Array.isArray(initialSchools)) return []
    return initialSchools.map((customer, index) => ({
      id: customer.id || customer.schoolId || `S${index + 1}`,
      name: customer.name || customer.schoolName || `客户${index + 1}`,
      contact: customer.contactPerson || customer.contact || '未填写',
      phone: customer.contactPhone || customer.phone || '未填写',
      salesman: customer.salesman || '未分配',
      region: customer.region || '未知',
      customerType: customer.customerType || '普通客户',
      dockingStatus: customer.dockingStatus || customer.status || '对接中',
      industryType: customer.industryType || '学校',
      schoolType: customer.schoolType || getRandomSchoolType(),
      isCooperation:
        typeof customer.isCooperation === 'boolean'
          ? customer.isCooperation
          : (customer.dockingStatus || customer.status) === '已对接',
      cooperationProjects:
        customer.cooperationProjects && customer.cooperationProjects.length > 0
          ? customer.cooperationProjects
          : getRandomProjects(),
  ...
  const derivedRegions = useMemo(() => {
    if (regionOptions && regionOptions.length > 0) return regionOptions
    const uniqueRegions = [...new Set(allSchools.map(customer => customer.region).filter(Boolean))]
    return uniqueRegions
  }, [regionOptions, allSchools])

  const regionData = useMemo(() => {
    if (!allSchools.length) return []
    const regionCount = allSchools.reduce((acc, school) => {
      if (!school.region) return acc
  ...
  useEffect(() => {
    setFilteredAllCustomers(allSchools)
  }, [allSchools])

  const calculateStatistics = useCallback((schools) => {
    if (!schools.length) {
      setIndustryStatistics({})
      return
    }
    const stats = schools.reduce((acc, school) => {
      const type = school.industryType || '学校'
    ...
    setIndustryStatistics({
      '学校': stats['学校'] || 0,
      '文旅': stats['文旅'] || 0,
      '政府+人工智能': stats['政府+人工智能'] || 0
    })
  }, [])

  useEffect(() => {
    calculateStatistics(allSchools)
  }, [allSchools, calculateStatistics])

  const filterData = useCallback((search, region, industryType, schoolType) => {
    let allCustomersData = [...allSchools]
  ...
  const handleSearch = (value) => {
    setSearchText(value)
    filterData(value, selectedRegion, selectedIndustryType, selectedSchoolType)
  }
  ...
  const showModal = (school = null) => {
    ...
    if (school) {
      form.setFieldsValue({
        ...
        industryType: school.industryType || '学校'
      })
    } else {
      form.resetFields()
    }
              ...
              >
                ...
                <Row gutter={16}>
                  ...
                  <Col span={12}>
                    ...
                    >
                      <Input placeholder="请输入区域，例如：广州市" />
                    </Form.Item>
                  </Col>
                </Row>
                ...
                <Form.Item name="isCooperation" valuePropName="checked">
                  <Checkbox>是否为合作客户</Checkbox>
                </Form.Item>

                {isCooperationChecked && (
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    >
                      <Select mode="multiple" placeholder="请选择合作项目">
...
function getRandomProjects() {
  const allProjects = ['硬件设备', '软件系统', '解决方案', '定制方案', '校史文化墙科', '校园科创文化墙', '党建文化墙', '美育文化墙']
